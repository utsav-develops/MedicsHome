...
const CreateComplaint = (props) => {
    ...
    const call_add_complaint = () => {
        ...
        axios({
            ...
            data: { work_id:work_id, complaint_category:complaint_category_id, complaint_sub_category:complaint_sub_category_id, subject:subject, description:description }
        })
            .then(async response => {
                setLoading(false);
                if(response.data.status == 1){
                    navigate_bill();
                    dropDownAlertRef.alertWithType('success', t('registeredSuccess'), t('complaintRegisteredSuccessfully'));
                }else{
    ...
    const navigate_bill = () => {
        navigation.navigate("Bill",{work_id: work_id})
    }
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={[styles.header]}>
                ...
                <View activeOpacity={1} style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontWeight: 'bold' }}>{t('complaint')}</Text>
                </View>
            </View>
            <ScrollView>
                    ...
                    >
                        ...
                        <View style={{ width: '100%', backgroundColor: colors.theme_bg_three, borderRadius: 10, padding: 20, marginTop:5, marginBottom:5 }}>
                            <View style={{ width:'100%', alignItems:'flex-start', justifyContent:'center'}}>
                                <Text style={{ color: colors.theme_fg_two, fontSize: f_m, fontWeight: 'bold' }}>{complaint_category_name} / {sub_category_data.complaint_sub_category_name}</Text>
                                <View style={{ margin:3 }} />
                                <Text style={{ color: colors.theme_fg_two, fontSize: f_s, fontWeight: 'bold' }}>{sub_category_data.short_description}</Text>
                            </View>
                        </View>
                        <View style={{ width: '100%', backgroundColor: colors.theme_bg_three, borderRadius: 10, padding: 20, marginTop:5, marginBottom:5 }}>
