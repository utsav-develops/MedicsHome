...
const DocumentUpload = (props) => {
  ...
  const [statusname, setStatusname] = useState(route.params.status);
  const lowerCaseStatus = statusname.toLowerCase();
  const [status, setStatus] = useState(statusMap[lowerCaseStatus]);
  ...
  const call_upload_document = async (data) => {
    ...
    RNFetchBlob.fetch('POST', api_url + image_upload, {
      ...
      {
        name: 'upload_path',
        data: 'staffs/speciality_documents'
      }
    ]).then(async (resp) => {
      //setLoading(false);
      ...
      setLoading(false);
      dropDownAlertRef.alertWithType('error', t('validationError'), t('errorOnUpload'));
    })
  }
    ...
    await axios({
      ...
      .then(async response => {
        ...
        if (response.data.status == 1) {
          ...
          setStatus(15);
          dropDownAlertRef.alertWithType('success', t('success'), t('successfullyUploadedDocument'));
        }
      })
      .catch(error => {
      console.error(error);
      ...
      return(
        <View style={{ margin:10 }}>
          <TouchableOpacity onPress={select_photo.bind(this)} activeOpacity={1} style={{ width: '100%', backgroundColor: colors.btn_color, borderRadius: 10, height: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ color: colors.theme_fg_two, fontSize: f_m, color: colors.theme_fg_three, fontWeight: 'bold' }}>{t('upload')}</Text>
          </TouchableOpacity>
        </View>
      )
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={{ marginBottom: 40, alignItems: 'center', justifyContent: 'center', flex:1 }}>
        ...
        <View style={{ margin: 10 }} />
        {status == 14 &&
          <Text style={{ fontWeight: 'bold', color: colors.theme_fg, fontSize: f_m }}>{t('uploadYourFile')}</Text>
        }
      </View>
      {loading == false ?
        <View>
