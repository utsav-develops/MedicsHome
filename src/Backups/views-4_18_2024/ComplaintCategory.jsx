...
const ComplaintCategory = (props) => {
    ...
    const navigate_complaint_sub_category = (id, complaint_category_name, work_id) => {
        navigation.navigate("ComplaintSubCategory", { complaint_category_id: id, complaint_category_name: complaint_category_name, work_id: work_id })
    }
            ...
            <View style={styles.animationHeader}>
                ...
                <View style={{ padding: 10, flexDirection: 'row' }}>
                    <View style={{ width: '70%', alignItems: 'flex-start', justifyContent: 'center' }}>
                        <Text numberOfLines={2} ellipsizeMode='tail' style={{ paddingLeft: 17, color: colors.theme_fg_three, fontSize: f_30, letterSpacing: 1, fontWeight: 'bold' }}>{t('howCanWeHelp')}</Text>
                    </View>
    ...
    const show_list = ({ item }) => (
        <TouchableOpacity onPress={navigate_complaint_sub_category.bind(this, item.id, item.complaint_category_name, work_id)} style={{ flexDirection: 'row', padding: 20 }}>
            ...
            <View style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
                <Text style={{ color: colors.theme_fg_two, fontSize: f_s, fontWeight: 'normal' }}>{item.complaint_category_name}</Text>
            </View>
        </TouchableOpacity>
    );
    return (
        <View style={styles.container}>
            ...
            <Animated.View opacity={headerHeight} style={[styles.header]}>
                ...
                <TouchableOpacity style={{ width: '83%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontWeight: 'bold' }}>{t('selectTheReason')}</Text>
                </TouchableOpacity>
            </Animated.View>
        </View>
    );
