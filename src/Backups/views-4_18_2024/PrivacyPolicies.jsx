...
const PrivacyPolicies = (props) => {
  ...
  const handleAcceptPolicy = (id, accepted) => {
    ...
    const handleSubmit = () => {
      ...
      .then(responses => {
        setLoading(false);
        dropdownRef.current.alertWithType('success', 'Success', 'Data sent successfully');
        go_back();
      })
      .catch(error => {
  ...
  useEffect(() => {
    ...
    if(from==="booking"){
        dropdownRef.current.alertWithType('error', 'Sorry', 'Please Accept policies first');
    }
  }, []);
        ...
        <View style={{ position: 'absolute', bottom: 5, width: '100%', height: 100, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
            {
              policydata.status === 0 ? ( // If policy status is 1
                counter === rowcount ? ( // If counter is rowcount
                  ...
                  >
                    <Text style={{ color: 'white', fontSize: f_m, fontWeight: 'bold' }}>Submit</Text>
                  </TouchableOpacity>
                  ...
                  <View style={{ width: '90%', backgroundColor: 'gray', borderRadius: 10, height: 50, alignItems: 'center', justifyContent: 'center' }}>
                    <Text style={{ color: 'black', fontSize: f_m, fontWeight: 'bold' }}>Submit</Text>
                  </View>
                )
              ) : ( // If policy status is not 1
                <View style={{ width: '90%', backgroundColor: 'gray', borderRadius: 10, height: 50, alignItems: 'center', justifyContent: 'center' }}>
                  <Text style={{ color: 'black', fontSize: f_m, fontWeight: 'bold' }}>{t('acceptedAt')} {policydata.accepted_at} {t('accepted_At')} </Text>
                </View>
              )
            }
