   ...
   const { t } = useLocalization();
    ...
    const call_delete_sos_contact = (data) => {
        ...
        axios({
            ...
            data: { workplace_id: global.id, contact_id: data.id }
        })
        .then(async response => {
            setLoading(false);
        ...
        return (
            <View style={styles.animationHeader}>
                ...
                <View style={{ padding: 30, alignItems: 'center' }}>
                    <Text numberOfLines={2} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_30, letterSpacing: 1, fontWeight: 'bold', textAlign: 'center' }}>Setup Emergency Contacts</Text>
                </View>
            </View>
        )
    }
        ...
        <TouchableOpacity style={{ flexDirection: 'row', padding: 10 }}>
            ...
            <View style={{ width: '80%', borderBottomWidth: 0.5, borderColor: colors.grey, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingBottom: 10 }}>
                <View style={{ width: '75%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text style={{ color: colors.text_grey, fontSize: f_l, fontWeight: 'normal' }}>{item.name}</Text>
                    <View style={{ margin: 3 }} />
                    <Text style={{ color: colors.text_grey, fontSize: f_s, fontweight: regular, letterSpacing: 1 }}>{item.phone_number}</Text>
                </View>
        ...
        <View style={styles.container}>
            ...
            >
                ...
                <TouchableOpacity onPress={navigate_add_contact.bind(this)} style={{ justifyContent:'center', alignItems:'center', marginTop:'20%' }}>
                    ...
                    <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_s, fontWeight: 'bold' }}>{t('addYourContact')}</Text>
                </TouchableOpacity>
                }
                {loading == true &&
            ...
            <View style={{ padding:10 }}>
                <TouchableOpacity onPress={ navigate_add_contact.bind(this) } activeOpacity={1} style={{ width: '100%', backgroundColor: colors.btn_color, borderRadius: 10, height: 50, flexDirection: 'row',  alignItems: 'center', justifyContent: 'center' }}>
                    <Text style={{ color: colors.theme_fg_two, fontSize: f_m, color: colors.theme_fg_three, fontWeight: 'bold' }}>{t('addContacts')}</Text>
                </TouchableOpacity>
            </View>
            ...
            <Animated.View opacity={headerHeight} style={[styles.header]}>
                ...
                <TouchableOpacity activeOpacity={1} style={{ width: '83%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontWeight: 'bold' }}>{t('emergencyContacts')}</Text>
                </TouchableOpacity>
            </Animated.View>
        </View>
    );
