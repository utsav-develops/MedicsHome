...
const AddEmergencyContact = (props) => {
    ...
    const call_add_sos_contact = async () => {
        ...
        axios({
            ...
            data: { workplace_id: global.id, name: contact_name, phone_number:contact_number }
        })
        .then(async response => {
            setLoading(false);
    ...
    <>
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{t('addEmergencyContact')}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>{t('emergencyNameNumber')}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
                    ...
                    {loading == false ?
                        <TouchableOpacity onPress={check_valid.bind(this)} activeOpacity={1} style={{ width: '100%', backgroundColor: colors.btn_color, borderRadius: 10, height: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                            <Text style={{ color: colors.theme_fg_two, fontSize: f_m, color: colors.theme_fg_three, fontFamily: bold }}>{t('add')}</Text>
                        </TouchableOpacity>
                        :
