...
const EditFirstName = (props) => {
    ...
    const call_profile_update = () => {
        ...
        axios({
            ...
            data: { id: global.id, first_name: first_name }
        })
            .then(async response => {
                setLoading(false);
    ...
    <>
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold  }}>{t('enterFirstname')}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal  }}>{t('needFirstName')}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
