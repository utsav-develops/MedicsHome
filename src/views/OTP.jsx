...
import { normal, bold, regular, f_xl, f_xs, f_m, update_phone, api_url } from '../config/Constants';
...
const OTP = (props) => {
    ...
    const check_valid = (val) => {
        if (val != otp) {
            dropDownAlertRef.alertWithType('error',  t('validationError'), t('enterValidOTP'));
        } else {
            navigate();
        }
    }
    ...
    <>
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{t('enterOTP')}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>{t('enterReceivedOTP')}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
