...
const DocumentUpload = (props) => {
  ...
  const show_button = () => {
    if (status == 14 || status == 17 || status == 15) {
      return(
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={{ marginBottom: 40, alignItems: 'center', justifyContent: 'center', flex:1 }}>
        <View style={{ height: '100%', width: '100%' }}>
          <Image source={path}  style={{ flex: 1, width: '100%', height: '100%' }}
           resizeMode="contain" />
        </View>
        <View style={{ margin: 10 }} />
        {status == 14 &&
          <Text style={{ fontFamily: bold, color: colors.theme_fg, fontSize: f_m }}>{t('uploadYourFile')}</Text>
        }
      </View>
      {loading == false ?
        <View>
