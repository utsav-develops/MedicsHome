...
const EditLastName = (props) => {
    ...
    const check_valid = () => {
        if (last_name) {
            ...
            dropDownAlertRef.alertWithType('error', t('validationError'), t('plzenterLastname'));
        }
    }
    ...
    const call_profile_update = () => {
        ...
        axios({
            ...
            data: { id: global.id, last_name: last_name }
        })
        .then(async response => {
            setLoading(false);
            global.last_name = last_name;
            dropDownAlertRef.alertWithType('success',  t('successfullyUpdated'), t('lastnameUpdated'));
            go_back();
        })
        .catch(error => {
            setLoading(false);
    ...
    <>
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold  }}>{t('enterLastname')}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal  }}>{t('needLastName')}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
