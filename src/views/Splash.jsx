...
const checkScheduleExactAlarmPermission = async () => {
  if (Platform.OS === 'android' && Platform.Version >= 33) {
    const permissionStatus = await check(PERMISSIONS.ANDROID.USE_EXACT_ALARM);

    if (permissionStatus === RESULTS.GRANTED) {
      call_emar();
    } else if (permissionStatus === RESULTS.DENIED) {
      const requestResult = await request(PERMISSIONS.ANDROID.USE_EXACT_ALARM);
      if (requestResult === RESULTS.GRANTED) {
        call_emar();
      } else {
        Alert.alert(
          'Permission Required',
          'Exact alarm permission is required to schedule alarms. Please enable it in settings or the medicine reminder feature doeesnot work.',
          [
            {
              text: 'Open Settings',
      ...
      // For any other result, prompt user to enable manually
      Alert.alert(
        'Permission Required',
        'Exact alarm permission is required. Please enable it in your phone settings.',
        [
          {
            text: 'Open Settings',
            onPress: () => Linking.openSettings(),
    ...
    // If not Android 14 or higher, call your call_emar function
    call_emar();
  }
        };
