...
import { bold, regular, api_url, get_profile, change_phone, check_phone, profile_picture_upload, profile_picture_update, img_url, f_xl, f_xs, f_m } from '../config/Constants';
...
import EditPhoneNumber from '../views/EditPhoneNumber';
...
const Profile = (props) => {
    ...
    const { t } = useLocalization();
     const navigate_otp = async (data_o) => {
        navigation.navigate('OTP', { otp: data_o.otp, phone_with_code: formattedValue, country_code: "+" + country_code, phone_number: value, id: 0, from: "profile" });
    }
    const insets = useSafeAreaInsets();
    const { isDarkMode, toggleTheme, colors } = useCustomTheme();
    console.log(global.profile_picture);
    ...
    const call_get_profile = () => {
        ...
        .then(async response => {
            ...
            setFormattedValue(response.data.result.phone_with_code);
            setValue(response.data.result.phone_number);
            set_country_code('977');
            global.profile_picture = response.data.result.profile_picture;
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={[styles.header]}>
                ...
                <View activeOpacity={1} style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontFamily: bold }}>{t('editProfile')}</Text>
                </View>
            </View>
            <ScrollView>
                {on_load == 1 &&
                    <View>
                        <TouchableOpacity activeOpacity={1} onPress={select_photo.bind(this)} style={{ alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderRadius: 65, padding: 2, width: 130, height: 130, borderColor: colors.grey, borderStyle: 'dotted', alignSelf: 'center', margin: 20 }}>
                            <View style={{ width: 120, height: 120 }} >
                                <Image style={{ height: undefined, width: undefined, flex: 1, borderRadius: 60 }} source={{ uri: data.profile_picture }} />
                            </View>
                        </TouchableOpacity>
                        <ScrollView>
                            <View style={{ alignItems: 'center' }}>
                                <Text style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{t('lilAboutYourself')}</Text>
                                <View style={{ margin: 20 }} />
                                <View style={{ width: '90%' }}>
                                    <View style={{ marginBottom: 20 }}>
                                        <Text style={{ color: colors.text_grey, fontSize: f_xs, fontFamily: bold }}>{t('firstName')}</Text>
                                        <View style={{ margin: 5 }} />
                                    ...
                                    <View style={{ marginBottom: 20 }}>
                                        <Text style={{ color: colors.text_grey, fontSize: f_xs, fontFamily: bold }}>{t('lastName')}</Text>
                                        <View style={{ margin: 5 }} />
                                    ...
                                    <View style={{ marginBottom: 20 }}>
                                        <Text style={{ color: colors.text_grey, fontSize: f_xs, fontFamily: bold }}>{t('email')}</Text>
                                        <View style={{ margin: 5 }} />
                                    ...
                                    <View style={{ marginBottom: 20 }}>
                                        <Text style={{ color: colors.text_grey, fontSize: f_xs, fontFamily: bold }}>{t('phoneNum')}</Text>
                                        <View style={{ margin: 5 }} />
                                        <TouchableOpacity activeOpacity={1} onPress={() => navigation.navigate('EditPhoneNumber')} style={{ flexDirection: 'row' }}>
                                            <View style={{ width: '15%', alignItems: 'center', justifyContent: 'center', backgroundColor: colors.theme_bg_three }}>
