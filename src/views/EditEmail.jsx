...
const EditEmail = (props) => {
    ...
    const call_profile_update = () => {
        ...
        .then(async response => {
            ...
            dropDownAlertRef.alertWithType('success', t('successfullyUpdated'), t('emailUpdated'));
            go_back();
        })
        .catch(error => {
            setLoading(false);
    ...
    <>
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{t('enterEmail')}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal  }}>{t('needtoenterEmail')}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
