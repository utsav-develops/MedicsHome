...
const WorkDetails = (props) => {
        ...
        axios({
            ...
            .then(async response => {
                ...
                if (response.data.status == 1) {
                    dropDownAlertRef.alertWithType('success', t('thankyou'), t('tipAddedSuccessfully'));
                    setTip(tip);
                }
            })
            .catch(error => {
    ...
    const call_work_details = () => {
        ...
        axios({
            ...
            .then(async response => {
                ...
                console.log("wo0rkr" + response.data.result);
    ...
    const move_chat = () => {
        navigation.navigate('Chat', { work_id: work_id, staff_name: data.work.staff.first_name });
    }
       ...
       <>
        ...
        <View style={styles.container}>
            ...
            <View style={{ flexDirection: 'row' }}>
                ...
                >
                    <TouchableOpacity activeOpacity={0} onPress={go_back.bind(this)} style={{ width: 40, height: 40, backgroundColor: colors.theme_bg_three, borderRadius: 25, alignItems: 'center', justifyContent: 'center', top: Platform.OS==='ios' ? insets.top :20, left: 20 }}>
                        <Icon type={Icons.MaterialIcons} name="arrow-back" color={colors.icon_inactive_color} style={{ fontSize: 22 }} />
                    </TouchableOpacity>

                </DropShadow>
                <TouchableOpacity
                  style={{top:20, width: 40,backgroundColor:colors.theme_bg_three,borderRadius:5,height: 40, marginLeft: '37%' , flexDirection: 'row',alignItems:'center', justifyContent: 'center'}}
                  onPress = {() => { map_ref.current.animateToRegion(userRegion, 1000)}}
                >
                  <Icon type={Icons.MaterialIcons} name="my-location" color={colors.icon_inactive_color} style={{ fontSize: 25 }} />
                </TouchableOpacity>

            </View>
            <BottomSheet sliderMinHeight={Platform.OS==='ios' ? 100 : 80} sliderMaxHeight={screenHeight - 5} bgColor={colors.lite_bg} isOpen>
                {(onScrollEndDrag) => (
                    <ScrollView onScrollEndDrag={onScrollEndDrag}
                    contentContainerStyle={{ paddingTop: 0, backgroundColor:colors.lite_bg }}
                    >
                        {on_load == 1 ?
                            <View style={{ padding: 10, paddingTop: 0 }}>
                                <View>
                                    <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: 20, fontFamily: normal}}>{t('yourOTPcode')} #{data.work.otp}</Text>
                                </View>
                                    ...
                                    <View style={{ flexDirection: 'row', width: '100%', marginTop: 20, marginBottom: 20 }}>
                                        <View style={{ width: '25%', alignItems: 'center', justifyContent: 'center' }}>
                                            <View style={{ height: 50, width: 50 }} >
                                                <Image style={{ height: undefined, width: undefined, flex: 1, borderRadius: 10,}} source={{ uri: data.work.staff.profile_picture }} />
                                            </View>
                                        </View>
                                        <View style={{ width: '40%', alignItems: 'flex-start', justifyContent: 'center' }}>
                                            <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: 17, fontFamily: bold }}>{data.work.staff.first_name}</Text>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5 }}>
                                                <Icon type={Icons.MaterialIcons} name="star" color={colors.warning} style={{ fontSize: 18 }} />
                                                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 13, fontFamily: regular }}>{data.work.staff.overall_ratings}</Text>
                                            </View>
                                        </View>
                                     ...
                                     <View style={{ alignSelf:'flex-end' }}>
                                        {data.work.start_time ? 
                                            <Text style={styles.title}>{t('started')} {(data.work.start_time)}</Text>
                                            : null
                                        }

                                        {data.work.start_time ? 
                                            <Text style={styles.title}>{t('EstimatedEndTime')} {(data.work.end_time)}</Text>
                                            : null
                                        }

                                     <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 13, fontFamily: regular }}> {data.work.speciality.number}</Text>
                                      </View>
                                </View>
                                <View style={{ borderBottomWidth: 0.5, borderColor: colors.grey }}>
                                    <View style={{ flexDirection: 'row', width: '100%', marginTop: 10, marginBottom: 10 }}>
                                        <View style={{ width: '33%', alignItems: 'center', justifyContent: 'center' }}>
                                            <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 13, fontFamily: regular }}>{t('hrs')}</Text>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5 }}>
                                                <Icon type={Icons.MaterialIcons} name="map" color={colors.theme_fg_two} style={{ fontSize: 22 }} />
                                                <View style={{ margin: 2 }} />
                                                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: 18, fontFamily: normal }}>{data.work.distance} hours</Text>
                                            </View>
                                        </View>
                                        <View style={{ width: '33%', alignItems: 'center', justifyContent: 'center' }}>
                                            <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 13, fontFamily: regular }}>{t('workType')}</Text>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5 }}>
                                                <Icon type={Icons.MaterialIcons} name="book-online" color={colors.theme_fg_two} style={{ fontSize: 22 }} />
                                                <View style={{ margin: 2 }} />
                                                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: 16, fontFamily: normal }}>{data.work.work_type_name}</Text>
                                            </View>
                                        </View>
                                        <View style={{ width: '33%', alignItems: 'center', justifyContent: 'center' }}>
                                            <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 13, fontFamily: regular }}>{t('rate')}</Text>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5 }}>
                                                <Icon type={Icons.MaterialIcons} name="local-atm" color={colors.theme_fg_two} style={{ fontSize: 22 }} />
                                                <View style={{ margin: 2 }} />
                                                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: 18, fontFamily: normal }}>{global.currency}{data.work.total}</Text>
                                            </View>
                                        </View>
                                    </View>
                                </View>
                                <View>
                                    <View style={{ width: '100%', marginTop: 20 }}>
                                        <TouchableOpacity activeOpacity={1} style={{ width: '100%', backgroundColor: colors.lite_bg }}>
                                            <View style={{ flexDirection: 'row', width: '100%', height: 50 }}>
                                                ...
                                                <View style={{ width: '90%', alignItems: 'flex-start', justifyContent: 'flex-start' }}>
                                                    <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 12, fontFamily: regular }}>{t('address')}</Text>
                                                    <View style={{ margin: 2 }} />
                                                    <Text numberOfLines={2} ellipsizeMode='tail' style={{ color: colors.theme_fg_two, fontSize: 13, fontFamily: regular }}>{data.work.pickup_address}</Text>
                                                    <View style={{ margin: 2 }} />
                                                </View>
                                            </View>
                                        </TouchableOpacity>
                                            ...
                                            <TouchableOpacity activeOpacity={1} style={{ width: '100%', backgroundColor: colors.lite_bg }}>
                                                <View style={{ flexDirection: 'row', width: '100%', height: 50 }}>
                                                    ...
                                                    <View style={{ width: '90%', alignItems: 'flex-start', justifyContent: 'flex-start' }}>
                                                        <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 12, fontFamily: regular }}>{t('address')}</Text>
                                                        <View style={{ margin: 2 }} />
                                                        <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_two, fontSize: 13, fontFamily: regular }}>{data.work.drop_address}</Text>
                                                    </View>
                                                </View>
                                            </TouchableOpacity>
                                        }
                                    ...
                                    <View style={{ width: '100%', marginBottom: 20 }}>
                                        {tip == 0 &&
                                            <View style={{ padding: 10 }}>
                                                <Text style={{ color: colors.theme_fg_two, fontSize: 20, fontFamily: bold }}>Add a tip for your staff</Text>
                                                <View style={{ margin: 2 }} />
                                                <Text style={{ color: colors.theme_fg_two, fontSize: 14, fontFamily: regular }}>The entire amount will be transferred to the bookingr. Valid only if you pay online.</Text>
                                                <View style={{ margin: 5 }} />
                                        ...
                                        <View style={{ flexDirection: 'row', width: '100%', marginBottom: 20 }}>
                                            ...
                                            {cancel_loading == false ?
                                                <TouchableOpacity onPress={showDialog.bind(this)} activeOpacity={1} style={{ width: '55%', backgroundColor: colors.error_background, borderRadius: 10, height: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                                                    <Text style={{ color: colors.theme_fg_two, fontSize: 16, color: colors.error, fontFamily: bold }}>{t('cancel')}</Text>
                                                </TouchableOpacity>
                                                :
            ...
            <Dialog
                ...
                dialogTitle={<DialogTitle title={t('reasontoCancelBooking')} textStyle={{ color:colors.theme_fg_two }} style={{backgroundColor:colors.lite_bg}} />}
                ...
                footer={
                    <DialogFooter>
                        <DialogButton
                            ...
                            style={{backgroundColor:colors.lite_bg}}
                        />
                    </DialogFooter>
                }
                onTouchOutside={() => {
            ...
            >
                <DialogContent style={{backgroundColor:colors.lite_bg}}>
                    <FlatList
